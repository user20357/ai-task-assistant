...
class AIChatManager(QObject):
    ...
    def _init_openai(self):
        ...
        try:
            ...
            self.error_occurred.emit(f"AI initialization failed: {e}")
    ...
    def get_current_step_guidance(self, detections: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, detection in enumerate(detections):
                elements_context += f"- {detection.get('label', 'Unknown')}: {detection.get('action', 'No action')}\n"
            
            # Ask AI what to do with these elements - emphasize RED SQUARES
            guidance_request = f"""I can see RED SQUARES highlighting these clickable elements:
{elements_context}

Based on our task "{self.current_task}", which RED SQUARE should I click next? 
Provide:
...
Keep it simple and actionable."""
    ...
    def get_help_response(self, help_request: str) -> str:
        ...
        try:
            help_context = f"User needs help during task {self.current_task}. They said: {help_request}"
            ...
            logger.error(f"Failed to get help response: {e}")
    ...
    def generate_task_steps(self, task_description: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to parse task steps JSON: {e}")
    ...
    def _parse_steps_from_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to parse steps from text: {e}")
            return [
                {
                    "step_number": 1,
                    "action": "click",
